    ...
    public class InventoryPacketHandler : IPacketHandler
    {
                ...
                else if (!Session.Character.IsSitting)
                {
                    ...
                    else
                    {
                        ...
                        if (timeSpanSinceLastSpUsage >= Session.Character.SpCooldown)
                        {
                            ...
                            {
                                if (Session.Character.Buff.Any(s => s.Card.BuffType == BuffType.Bad))//Fixxed by xDanii
                                {
                                    Session.SendPacket(
                                        UserInterfaceHelper.GenerateMsg(
        ...
        public void Upgrade(UpgradePacket upgradePacket)
        {
            ...
            switch (uptype)
            {
                ...
                case 7:
                    inventory = Session.Character.Inventory.LoadBySlotAndType<ItemInstance>(slot, inventoryType);
                    if (inventory != null)
                    {
                        if (inventory.Item.EquipmentSlot == EquipmentType.Armor || inventory.Item.EquipmentSlot == EquipmentType.MainWeapon || inventory.Item.EquipmentSlot == EquipmentType.SecondaryWeapon)
                        {
                            ...
                            //FixedUpMode isFixed = FixedUpMode.HasAmulet; //isFixed wir nicht benutzt,keine Variable (Vielleich das Problem)
        ...
        private void RemoveSp(short vnum)
        {
            if (Session?.HasSession == true && !Session.Character.IsVehicled)
            {
                ...
                Session.SendPacket(Session.Character.GenerateCond());
                Session.SendPacket(Session.Character.GenerateLev());
                // Cooldown der Sp an/ausziehen.
                Session.Character.SpCooldown = 5;
