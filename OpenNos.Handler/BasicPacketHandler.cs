    ...
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        public void CharacterOptionChange(CharacterOptionPacket characterOptionPacket)
        {
            switch (characterOptionPacket.Option)
            {
                ...
                case CharacterOption.AllowRevivalPet:
                    Session.Character.AllowRevivalPet = IsActive;
                    Session.SendPacket(UserInterfaceHelper.GenerateMsg(
                        Language.Instance.GetMessageFromKey(Session.Character.AllowRevivalPet
                            ? "REVIVAL_THIS_PET"
                            : "NOT_REVIVAL_THIS_PET"), 0));
                    break;

                case CharacterOption.AllowRevivalPartner:
                    Session.Character.AllowRevivalPartner = IsActive;
                    Session.SendPacket(UserInterfaceHelper.GenerateMsg(
                        Language.Instance.GetMessageFromKey(Session.Character.AllowRevivalPartner
                    ...
                    if (Session.HasCurrentMapInstance)
                    {
                        Session.CurrentMapInstance.Monsters.Where(m => m.MapMonsterId == (int)ncifPacket.TargetId)
                            .ToList().ForEach(monster =>
                            {
                                ...
                                Session.SendPacket(
                                     $"st 3 {ncifPacket.TargetId} {monsterinfo.Level} {monsterinfo.HeroLevel} {(int)(monster.CurrentHp / (float)monster.MaxHp * 100)} {(int)(monster.CurrentMp / (float)monster.MaxMp * 100)} {monster.CurrentHp} {monster.CurrentMp}{monster.Buff.GetAllItems().Aggregate(string.Empty, (current, buff) => current + $" {buff.Card.CardId}")}");
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            if (ServerManager.Instance.Configuration.WorldInformation)
            {
                ...
                Session.SendPacket(
                    Session.Character.GenerateSay($"Welcome on EastNos", 11));
                Session.SendPacket(Session.Character.GenerateSay("-----------------------------------------------",
                    10));
            }
