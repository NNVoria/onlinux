    ...
    public class SpecialItem : Item
    {
        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
            ...
            switch (Effect)
            {
                ...
                // Noch Crash Vorhanden bitte erstmal lassen Danke (Sasuke)
                /*
                case 666:
                ...
                case 6969: // this is imaginary number I = √(-1)
                    ...
                    if (rel != null)
                    {
                        if (rel.CharacterId == session.Character.CharacterId)
                        {
                            session.Character.DeleteRelation(rel.RelatedCharacterId);
                        }
                        else
                        {
                            session.Character.DeleteRelation(rel.CharacterId);
                        }
                ...
                case 34: // this is imaginary number I = √(-1)
                    ...
                    session.SendPacket(UserInterfaceHelper.GenerateDialog(
                        $"#marriage^2{packetsplit[3]} #marriage " +
                        $"{string.Format(Language.Instance.GetMessageFromKey("MARRIAGE_QUESTION"), ServerManager.Instance.GetSessionByCharacterId(long.TryParse(packetsplit[3], out long charid) ? charid : 0).Character.Name)}"));
                    break;
                ...
                case 203:
                    if (!session.Character.IsVehicled)
                    {
                        ...
                        {
                            session.SendPacket(
                                UserInterfaceHelper.GenerateGuri(10, 2, session.Character.CharacterId, 1));
                        }
                    }
                ...
                case 5018:
                    ...
                    session.Character.GiftAdd(8104, 1);
                    session.Character.GiftAdd(8100, 1);
                    session.Character.GiftAdd(8104, 1);
                    session.Character.GiftAdd(8100, 1);
                    session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                    break;
                ...
                //Riesiges Warenhaus
                case 5380:
                    session.Character.GiftAdd(3116, 1);
                    session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                    break;
                ...
                //Göttliche Fee
                case 5279:
                    int rndm = ServerManager.RandomNumber(0, 3000);
                    short[] itemvnum = null;
                    if (rndm < 1900)
                    {
                        itemvnum = new short[] { 4129, 4130, 4131, 4132 };
                    }
                    else
                    {
                        itemvnum = new short[] { 4129, 4130, 4131, 4132 };
                    }
                    session.Character.GiftAdd(itemvnum[ServerManager.RandomNumber(0, 4)], 1);
                    session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                    break;
