...
namespace OpenNos.GameObject
{
    public class ItemInstance : ItemInstanceDTO
    {
        ...
        public void RarifyItem(ClientSession session, RarifyMode mode, RarifyProtection protection, bool isCommand = false, byte forceRare = 0)
        {
            ...
            if (rnd < rare[10] && Item.IsHeroic && protection == RarifyProtection.RandomHeroicAmulet || rnd < rare[10] && Item.IsHeroic && protection == RarifyProtection.HeroicAmulet && !(protection == RarifyProtection.RandomHeroicAmulet && Rare >= 8 || protection == RarifyProtection.HeroicAmulet && Rare >= 8))
            {
                rarify(8);
            ...
            else if (rnd < rare[9] && !(protection == RarifyProtection.Scroll && Rare >= 7))
            {
                rarify(7);
                GenerateHeroicShell(protection);
                ...
                if (mode != RarifyMode.Drop && session != null)
                {
                    switch (protection)
                    {
                        ...
                        case RarifyProtection.RandomHeroicAmulet:
                            ItemInstance amulet = session.Character.Inventory.LoadBySlotAndType<ItemInstance>((short)EquipmentType.Amulet, InventoryType.Wear);
        ...
        public void UpgradeItem(ClientSession session, UpgradeMode mode, UpgradeProtection protection, bool isCommand = false, FixedUpMode hasAmulet = FixedUpMode.None)
        {
            ...
            if (Upgrade < 10)
            {
                ...
                if (Rare >= 8)
                {
                    upfix = new byte[] { 0, 0, 10, 15, 20, 20, 20, 25, 6, 1 };
                    upfail = new byte[] { 0, 0, 0, 5, 20, 40, 60, 70, 90, 98 };
                ...
                {
                    upfix = new byte[] { 0, 0, 10, 15, 20, 20, 20, 5, 5, 1 };
                    upfail = new byte[] { 0, 0, 0, 5, 20, 40, 60, 85, 90, 98 };
