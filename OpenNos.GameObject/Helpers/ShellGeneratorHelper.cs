...
namespace OpenNos.GameObject.Helpers
{
    ...
    public class ShellGeneratorHelper : Singleton<ShellGeneratorHelper>
    {
        ...
        public List<ShellEffectDTO> GenerateShell(byte shellType, int shellRarity, int shellLevel)
        {
            int w;
            List<int> factor;
            int letterMultiplier = 1;
            List<ShellEffectDTO> shellOptions = new List<ShellEffectDTO>();
            ...
            for (int g = 3; g <= 23; g += 2)
            {
                ...
                shellOptions.Add(new ShellEffectDTO
                {
                    EffectLevel = (OpenNos.Domain.ShellEffectLevelType)optionLevel,
                    Effect = (byte)optionType,
                    Value = (byte)optionValue,
                });
            }
            return shellOptions;
        }
