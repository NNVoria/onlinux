...
namespace OpenNos.GameObject
{
    public class Mate : MateDTO
    {
        ...
        private const short seed = 1012;

        private const short petGuardian = 2089;

        private const short partnerGuardian = 2329;
        ...
        public DateTime LastAttack { private get; set; }

        public DateTime LastDefence { private get; set; }

        private DateTime LastRefreshStat { get; set; }
        ...
        public void RevivalMate()
        {
            IsAlive = false;
            Hp = 1;
            Owner.Session.SendPacket(GenerateCtl());
            Owner.Session.SendPackets(Owner.GeneratePst());
            bool isPet = MateType.Pet == MateType;
            bool isAllowRevival = isPet ? Owner.AllowRevivalPet : Owner.AllowRevivalPartner;
            if (isPet && isAllowRevival)
            {
                RevivalMate(MateIsPet: true);
            }
            else if (!isPet && isAllowRevival)
            {
                RevivalMate(MateIsPet: false);
            }
            else
            {
                Owner.Session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("NOSMATE_DIE"), 10));
                Owner.Session.SendPacket("petski -1");
                IsTeamMember = false;
                Owner.Session.SendPacket(GenerateOut());
                Owner.Session.SendPacket(Owner.GeneratePinit());
                IsAlive = true;
            }

            void RevivalMate(bool MateIsPet)
            {
                if (Owner.Inventory.CountItem(MateIsPet ? petGuardian : partnerGuardian) > 0)
                {
                    Owner.Session.SendPacket(UserInterfaceHelper.GenerateMsg(
                                           Language.Instance.GetMessageFromKey("GUARDIAN_PROTECT_YOUR_NOSMATE"), 10));
                    Owner.Session.Character.Inventory.RemoveItemAmount(MateIsPet ? petGuardian : partnerGuardian);
                    Hp = MaxHp / 2;
                    Mp = MaxMp;
                    Owner.Session.SendPacket(GenerateOut());
                    IsAlive = true;
                ...
                else if (Owner.Inventory.CountItem(seed) > 4)
                {
                    Owner.Session.SendPacket(UserInterfaceHelper.GenerateMsg(
                                           string.Format(Language.Instance.GetMessageFromKey("NOSMATE_DIE_REVIVAL_IN_MIN"), 3), 10));
                    Owner.Session.SendPacket(Owner.Session.Character.GenerateSay(
                                           string.Format(Language.Instance.GetMessageFromKey("SEED_USED"), 5), 10));
                    Owner.Session.Character.Inventory.RemoveItemAmount(seed, 5);
            ...
            if (isPet)
            {
                ...
                if (Loyalty < 150)
                {
                    Owner.Session.SendPacket(UserInterfaceHelper.GenerateSay(Language.Instance.GetMessageFromKey("LOYALTY_TOO_LOW"), 10));
                }
            }
            Owner.Session.SendPacket(GenerateScPacket());
        }
        ...
        {
            if (LastDefence.AddSeconds(3) <= DateTime.Now && LastAttack.AddSeconds(2) <= DateTime.Now)
            {
                Owner.Session.SendPacket($"pst 2 {MateTransportId} {(int)MateType} {(IsAlive ? (int)(Hp / (float)MaxHp * 100) : 1)} {(IsAlive ? (int)(Mp / (float)MaxMp * 100) : 100)} {Hp} {Mp} 0 0 0");
            }
            if (Owner.Miniland.MapInstanceId == Owner.MapInstance.MapInstanceId && Loyalty < 1000 && LastRefreshStat.AddSeconds(5) <= DateTime.Now)
            {
                Loyalty += 100;
                if (Loyalty >= 1000)
                {
